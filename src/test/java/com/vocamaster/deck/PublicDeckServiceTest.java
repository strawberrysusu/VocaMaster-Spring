...
class PublicDeckServiceTest extends AbstractIntegrationTest {
    ...
    private Deck pub;       // PUBLIC, 제목에 "토익"
    private Deck pub2;      // PUBLIC, 설명에 "일본어"
    private Deck unlisted;  // UNLISTED, 제목에 "토익" — 검색엔 절대 안 나와야 함
    private Deck priv;      // PRIVATE, 설명에 "토익" — And/Or 괄호 사고 나면 이게 새어 나옴
    ...
    void search_publicOnly() {
        Page<PublicDeckResponse> result = publicDeckService.search(null, 0, 20);
    ...
    void search_keyword_noLeak() {
        // "토익"은 priv의 설명, unlisted의 제목에도 있음 — PUBLIC인 pub만 나와야 함
        Page<PublicDeckResponse> result = publicDeckService.search("토익", 0, 20);
    ...
    void search_keyword_matchesDescription() {
        Page<PublicDeckResponse> result = publicDeckService.search("일본어", 0, 20);
    ...
    void search_blankKeyword() {
        assertEquals(2, publicDeckService.search("   ", 0, 20).getTotalElements());
    }
