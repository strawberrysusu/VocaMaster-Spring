...
public interface QuizQuestionRepository extends JpaRepository<QuizQuestion, Long> {
    ...
    // 덱 통계용 (8/28 Mustache 청소 때 attempts→세션 기반 전환) — 답한 문항만 센다
    @Query("select count(q) from QuizQuestion q where q.session.deck.id = :deckId and q.session.user.id = :userId and q.answeredAt is not null")
    long countAnswered(@Param("deckId") Long deckId, @Param("userId") Long userId);

    @Query("select count(q) from QuizQuestion q where q.session.deck.id = :deckId and q.session.user.id = :userId and q.isCorrect = true")
    long countAnsweredCorrect(@Param("deckId") Long deckId, @Param("userId") Long userId);
