...
**9/3 이번 주 묶음 1개 사용 — 실사용에서 드러난 결함 3건 (PR `fix/ux-mobile-row-import-preview`)**
- 계기: 사용자가 9/2 국비에서 덱 172(사설신문 단어)를 만들고 5줄 붙여넣기 → 다음 날 카드 0장. Codex 운영 읽기 전용 조사: 덱 생성 200 → `/import/preview` 200 → **`/decks/172/import` 요청 0회**, DB·binlog에 INSERT/DELETE 0건. 저장 후 유실이 아니라 **미리보기를 저장으로 오해**한 건. 코드 대조: 미리보기 목록이 덱 카드 목록과 같은 `.word-row` 모양이고 등록 버튼은 목록 **위**에만 있었음.
- ① `ImportCards.tsx`: 버튼에 단계 번호(1. 미리보기 / 2. N장 실제 등록), 목록 위 「미리보기 — 아직 저장되지 않았습니다」 띠(`.preview-notice`), 목록 **아래**에 같은 등록 버튼. 저장 조건·snapshot·previewStale 로직 무변경. 상한 대상(UX 개선)으로 계상.
- ② `DeckDetail.tsx` + `index.css`: 폰에서 뜻 칸이 한 글자 폭으로 짜부되던 것(운영 스크린샷) — 원인은 데스크톱 flex 한 줄에 모바일 규칙 0개. 덱 상세 일반 행에만 `.owned-card-row` modifier 붙여 768px 이하에서 2줄 grid(1행 번호|단어|액션, 2행 뜻). 전역 `.word-row`(가져오기·공개 덱) 무변경. 320/375/390/430px 렌더 + 가로 스크롤 0 확인.
- ③ `DeckDetail.tsx`: 「⭐만」→「⭐ 별표 복습」 문자열만. 이 버튼은 `/study?deckId=N&starredOnly=1`로 가는 **별표만 Leitner 복습의 유일한 진입점**(Study에 카드별 별표 토글은 있으나 '별표만 골라 시작' 범위 선택은 없음). 삭제 제안은 기각. 오표기 정정(상한 밖).
- 검증: oxlint 0 / `tsc -b && vite build` 통과 / vite 프리뷰 + fetch 목으로 브라우저 스모크 — 미리보기만으로는 `/import` 요청 없음, 아래 등록 버튼 클릭 시 `/decks/172/import` 1회. 백엔드·파싱·운영 데이터 무변경.
- **9/3 결과**: PR #5 squash merge(9e0d93c) → CD success → 서빙 JS 해시 = 로컬 빌드 해시 일치. 사용자가 덱 172에 5줄 재등록해 카드 5장 확인(사건 종결). 사용자 피드백: **아래쪽 등록 버튼은 노란 띠가 있으니 어색하다 → 제거 예정(다음 주 A 묶음에 포함, 별도 배포 안 함)**.
- **A는 같은 날(9/3 오후) 처리 — 사용자 "다 해야 하는 거 아니냐"** (PR `fix/mobile-edit-row-create-form`). 규칙 해석: ①② 폰에서 편집·추가가 **안 되는** 결함 = 장애 수리(상한 밖), ③ = 오늘 묶음(PR #5)의 되돌림 정정. 새 기능 0.
  - ① `.word-row.row-edit` 768px 이하 grid(번호 | 입력칸 세로 3개 + 저장·취소). 375px 실측: 입력칸 40px 폭 → 248px.
  - ② 원인 실측: 모바일 `.create-row { flex-direction: column }`인데 `.reading-input { flex: 0 1 220px }`의 basis가 **높이** 220px로 먹혀 폼 401px. 뒤쪽 규칙보다 특이도 높은 `.create-row input.reading-input { flex: 0 0 auto }`로 덮고 폰에서 입력칸 테두리 부여. 401→225px. `.create-row`를 쓰는 Decks·Explore 검색줄에도 같은 규칙(216px, 정상).
  - ③ ImportCards 아래쪽 등록 버튼 제거, 노란 띠는 유지. 미리보기 후 등록 버튼 1개 실측.
  - 데스크톱 1200px 회귀: 수정 행 flex·입력 207px, 추가 폼 68px 한 줄·테두리 0 — 기존과 동일.
  - ~~**또 발견, 미수정**: 가져오기 화면의 구분자 버튼 6개가 폰에서 세로 글자 기둥으로 깨짐~~ → **같은 날 수리(사용자 지시)**: `.sort-tabs`·`.setup-row`에 `flex-wrap: wrap`, 버튼에 `white-space: nowrap`. 375px 실측 3줄·37px, 1200px 1줄 동일. `.sort-tabs`를 쓰는 탐색·퀴즈·타이핑·듣기·설정 탭도 같은 규칙(넘칠 때만 줄바꿈이라 무해).
- **Codex 최종 검토 (9/3 저녁)**: 운영 상태 OK(lint·build·CI·deploy·health·smoke 전부 확인). 지적 2건 → P2 `ImportCards` 등록 요청 중(busy) textarea가 잠기지 않음 — 느린 회선에서 등록 누른 뒤 내용을 바꾸면 화면은 B, 서버엔 스냅샷 A 저장(데이터 오염은 아님: 미리보기한 것이 저장됨). 수리 = textarea `disabled={busy}` 1줄, **다음 코드 묶음에 포함(오늘 배포 안 함)**. P3 문서 모순 → 위 취소선으로 정리(이 커밋).
- **동결 판정 (Codex, Claude 동의)**: 계획은 PR #5 하나였는데 같은 날 #6·#7까지 3개 배포. 수정 자체는 정상이나 **동결 장치가 또 풀림** — 이번엔 "결함"이라는 사유로. 규칙 보강 제안: **상한 밖 '장애'는 모든 기기에서 기능이 안 되는 것만**, 특정 화면 폭의 결함은 주간 묶음에 포함. **다음 주 코드 변경 0(이번 주 초과분 상쇄), B(카드 상자화·하단 고정 버튼)는 11월 재평가로.** ← **9/3 사용자 확정("그렇게 하자")**. 추가 기록: `.gitignore` 커밋 2건이 docs 필터(docs/·*.md)에 안 걸려 **코드 무변경 재배포 2회** 발생, 그중 하나의 컨테이너 교체 구간에서 사용자가 502 목격(28초 내 복구, 해시 동일). 교훈: md 아닌 잡무 파일은 코드 PR에 묶기. 배포 생략 필터를 `.gitignore`·`.github/**`·`docs/**`로 넓히는 것은 CI 3종 세트 정비에 포함.
- ~~**다음 주 묶음 A**~~ → 9/3 완료. ~~**다음 주 = B**~~ → 위 판정 참조. (참고) B: Claude Design 모바일 프로토타입(9/3 `VocaMaster 리디자인.html`, 375 토글) 중 프론트만으로 되는 3개 — 카드 상자화·하단 고정 학습 버튼·모드 세그먼트. 프로토타입의 카드별 「박스 N·다음 복습」 칩과 덱 진행바는 **백엔드 필요 → 11월**.
- ~~**작업 중 발견, 구현 안 함 (백로그)**~~ → **ⓐⓑ는 같은 날 PR #6에서 수리됨(위 A 항목). 아래는 발견 당시 기록.** ⓐ 덱 상세 **카드 수정 행**(`.row-edit`)은 375px에서 입력칸 3개가 40px 폭으로 짜부돼 사실상 입력 불가 — 이번 범위(일반 행) 밖이라 미수정. ⓑ 덱 상세 **카드 추가 폼**(`.create-row`)이 모바일에서 세로 397px로 늘어져 목록이 한 화면 아래로 밀림. 둘 다 모바일 전용 CSS 결함, 다음 묶음 후보. ⓒ 프리뷰 에뮬레이터에서 폭 768.57px(소수)일 때 `max-width:768`·`min-width:769` 둘 다 불일치 — 실기기 정수 폭에선 해당 없음(환경 특성, 기록만).

**9/5(토) Codex 최종 감사 10건 — 검증·판정 (코드 무변경, docs only)**
- 계기: Codex 최종 점검 보고서(`.codex-resume-audit/final-audit-20260905.md`, gitignored) — F1~F8·A1·A2 10건, "F1~F3 P1". Claude 검증: 건별 검증 에이전트 + 존재 반박 + 긴급성 반박(27 에이전트). 실측 = F2/F3 임시 JUnit(`new ImportService(null,null).preview()` 호출, 실행 후 삭제) / F1 Codex Playwright 스크립트 재실행(`discardEnabled:true → draftAfterLateSuccess:null` 재현) / A1 실제 `client.ts`를 Node에서 fetch 순서 제어 실행. 앱 코드·운영 무변경, 트리 클린.
- **결과: 10/10 기전 사실.** Claude 1차 판정 "동결 예외 0건"은 **철회** — 규칙 원문(위 119행 "데이터 유실")을 "저장 데이터"로 좁혀 읽은 것(Codex 지적, 수용). 정확한 분류:
  - **F1 [유실 범주 — 미전송 답안]** `Study.tsx:407` 버리기 버튼에 `disabled={submitting}` 없음. 제출 중 버리기 → 새 세션 B에 답 → A의 **늦은 성공 응답**이 `clearDraft()`·`setAnswers({})`로 B를 메모리·sessionStorage에서 삭제. 서버 기록은 정상(A 1회 반영, B는 `submitting`이 안 풀려 전송 불가). 조건 = **수 초 이상 걸리는 제출**(느린 회선·재배포 정지) 중 버리기 클릭 후 답하기. 늦은 **실패** 응답은 B를 안 지움. 수리 1줄(+응답 세대 검사 4줄 선택).
  - **F2 [오저장 — 미리보기에 보임]** `ImportService.java:128` `"\t".isBlank()==true` → 탭 명시가 자동 감지로. 탭 없는 줄은 `-`,`,` 등으로 쪼개져 성공 처리(`mother-in-law meaning` → mother/in/law meaning). 텍스트 경로만(파일 경로는 항상 `separator:''`). 수리 `isEmpty` 1줄.
  - **F3 [오저장 — 파일 경로는 무예고]** `ImportService.java:132` 줄 전체 `trim()`이 양끝 탭 제거 → **첫 칸 또는 마지막 칸이 빈 줄**이 2칸으로 통과(읽기→단어 / 읽기→뜻, `failedCount 0`). 텍스트 경로는 미리보기 루비·뜻에 보이지만 **파일 가져오기는 미리보기 없이 저장**. 기존 카드 훼손은 없음. 수리: `\r` 제거·빈 줄 skip 후 원문 split, 칸별 trim(~3줄).
  - **F8 [표시 불일치]** 9/3 P2 그대로 — textarea `disabled={busy}` 1줄. 서버엔 스냅샷 A만 감(안전).
  - **A1 [보안 범주 — 같은 기기]** 만료 access로 페이지 진입(TopNav가 매 페이지 `/folders`·`/decks` 호출 → 트리거 면은 Settings만이 아님) → refresh 진행 중 로그아웃 → 늦은 refresh 응답이 `client.ts:43 setToken`. 두 순서: ⓐ access만 재저장 = ≤1h, 무상태 access 설계 안 / ⓑ **refresh `Set-Cookie`가 로그아웃 응답 뒤 도착하면 14일 쿠키 + DB `R_new` 활성 = 로그아웃 무효화(설계 밖)**. 반대 순서(로그아웃이 먼저 처리)는 재사용 탐지로 **전 기기 refresh 폐기**. 원격 악용 불가, 실서버+브라우저 재현 미실시. "원래 1시간 정책"으로 뭉개지 않는다.
  - **A2 [편의]** 두 탭 동시 refresh → 패자를 재사용 공격으로 판정 → 전 기기 refresh 폐기 → 재로그인. fail-closed(더 잠그는 방향), 보안 약화 아님. 수리는 탭 간 조정(Web Locks 등 ~15줄).
  - **F4~F7 [막다른 화면·안내 없음]** F4 한 세션 1000개 초과 답안(>1000장 덱 또는 due 전체)만 해당 → 400·동결본 재시도 불가·버리기만. F5 미제출 초안에 삭제 카드 → 404 전체 롤백·버리기만(서버 정상). F6 파일 실패 후 **동일** 파일 재선택 시 native input 미초기화(`e.target.value=''` 1줄). F7 `tts.ts speakTimes` onerror 미처리 → 듣기 재생 멈춤·안내 없음(`cancel()` 가드 필수, ~10줄).
- **결정 (9/5 사용자·Codex·Claude 합의): 공부 우선. "영향 작고 조건 드물어 감수하고 미룸"이지 "동결 예외 없음"이 아니다.** 9/8 주 코드 0 유지(9/3 결정). **9/14 주 묶음 1개 = F1·F2·F3·F8 + 회귀 테스트.** 완료 기준은 줄 수가 아니라 **재현 차단**: `ImportServiceTest`에 명시 탭·첫 칸 빈·마지막 칸 빈 3케이스가 실패 줄로 / Codex `final-20260905.mjs` 재실행에서 `discardEnabled:false`·`editable:false`(스크립트가 버리기 클릭을 전제하므로 그 단계는 조정). **A1은 편의 백로그와 분리, 별도 검증 일정**(실서버+브라우저 재현 먼저 → epoch 가드 ~12줄 + logout이 진행 중 refresh를 기다리게; `removeItem` 추가만으로 완료 선언 금지). F4~F7·A2 백로그(11월 이후). 추가 전수조사·기능 확장 금지.
- **회피 (9/16 PR #8 이후 정정, Codex 검산)**: ~~보내는 중엔 버리기 X~~(F1 수리로 해제) / 텍스트 가져오기는 미리보기 **행**을 확인(습관으로 유지) / **빈 칸 섞인 시트 데이터는 파일 가져오기 X — 유지**: `ImportFiles.tsx`가 `separator:''`를 박아 보내 파일 경로는 자동 감지 고정(백엔드 `ImportFileRequest`엔 separator 필드 있음 → 수리 = 프런트 구분자 선택기, 위 89행 ② fail-closed 가져오기).
- **9/15 묶음 편입 (사용자 확정)**: 학습 세션 진척 막대가 답한 수 기준이라 답 없이 '다음'·별표·'이전'에서 멈춰 보임(사용자 폰 신고) → 위치 기준 `(idx+1)/total`로 수리(`b91f940`, aria-valuetext에 위치·답한 수). 표시 결함이라 9/3 규칙상 단독 배포 대상 아님 → 이번 주 묶음에 편입. 검증: 빌드 결과 + API 목 Playwright 8단계 — 수정 전 막대 0%·20%·40%에서 멈춤, 수정 후 20→40→60→40(이전)→60→60(별표)→80→100% 전 단계 위치 일치, pageError 0. **묶음 남은 일: F2·F3 + 회귀 테스트** → 끝나면 PR 1개로 배포.
- **9/16 F2·F3 + 회귀 테스트 완료 → 9/14 주 묶음 종결 (PR #8)**: F2 `ImportService:128` `isBlank`→`isEmpty` — 탭 하나는 공백 문자라 `isBlank`가 true, 명시한 탭이 자동 감지로 빠져 하이픈 등으로 쪼개졌다. F3 `ImportService:132` 줄 전체 `trim` 대신 `
`만 뗀 원문을 `split`, `trim`은 빈 줄 판정에만 — 양끝 탭이 지워지면 빈 칸의 유일한 흔적이 사라져 3칸이 2칸으로 통과했다. 회귀 테스트 `ImportServiceTest` 2건(F2: 탭 명시 + `mother-in-law meaning` → 실패 1 / F3: 첫 칸 빈·마지막 칸 빈·정상 3줄 → 정상 1·실패 2). **재현 증명**: 수리만 stash하고 `-PnoFailFast`로 실행 → 10개 중 정확히 F2·F3 2개 실패, 복구 후 10/10 (로컬 Temurin 17 = CI와 동일). 주의: `failFast` 기본 켜짐이라 첫 실패에서 멈춰 F3가 skipped로 보였다 → 재현 증명 땐 `-PnoFailFast` 필수. 사용자 타이핑(F2 자력, F3 도움 — 원본/사본 두 변수), 오타·주석 정리는 Claude. DTO 주석 "blank이면 자동 감지"도 "빈 문자열이면"으로 정정. **배포 검증(9/16 16:1x)**: PR #8 squash `1ddb514` → run 35066575290 build·deploy success(앱 변경 감지·Docker build·전송·compose 재기동·헬스·HTTP 스모크 전부 통과) → 컨테이너 `vocamaster-app-app-1 Up 2 minutes (healthy)` → 서빙 JS 해시 `index-BcUxcQes.js` = 로컬 빌드(배포 전 `index-B8zQQESR.js`). CI 테스트 집계 tests=197 failures=0. 사용자 폰 확인 항목: 가져오기에서 구분자 '탭' 선택 후 탭 없는 줄이 실패 줄로 뜨는지.
- ~~**9/21 주 묶음**~~ → **9/17 PR #9에 흡수(아래 항목)**. 원래 계획(9/14 사용자 결정 "다음 주에 한 번에"): 월 9/21 코드 0(10:00 산업기사 접수) / 화 9/22 CSS 2개 — 덱 상세 행 버튼 흐림(`index.css` `.row-actions{opacity:.35}`→1, 별 기본색 진하게)·복습 후리가나 `.reading` 15px→clamp(단어가 폰에서 clamp라 읽기도 clamp여야 역전 안 됨) / 수 9/23 Study 키보드 단축키(Space 뒤집기·A 몰라요·D 알아요·S 별표·←→ 이전다음, 함정 4: 한글 IME는 `e.code`·카드 버튼 포커스 Space 이중 실행은 `preventDefault`·`e.repeat` 무시·submitting/pendingRetry/reviewing 잠금) / 목 9/24 검증(Playwright 키 입력, 한글 IME는 수동)·CHECKLIST·PR·CD·해시. 새 브랜치 1개, PR 1개, 전부 사용자 타이핑.
- **9/17 사용자 결정 — 9월 남은 묶음을 하루에 (PR #9)**: "국비 시간에 여유가 생겨 지금이 마무리 타이밍. 동결은 생각 없는 기능 확장을 막으려던 것이고 이건 소규모 조작 개선" → 위 9/21 주 계획 3건 + 폰으로 쓰다 나온 제안 4건(9/17 아침, 스크린샷 근거)을 브랜치 하나로. ① `.row-actions` opacity .35→1, `.star-btn` 색 muted ② `.study-card .reading` 15px→clamp(17px,4.2vw,24px) ③ 뒤집으면 자동 발음(`Settings.autoSpeak`, 기본 켬) + 복습 화면 🔊 스위치(발음 듣기 버튼 대체, 다시 듣기 = 다시 뒤집기) + 설정 화면 토글(같은 값) + 카드 이동·화면 이탈 시 `stopSpeaking` ④ 키보드 단축키 Space·A·D·S·←→ (`e.code` 기준, 버튼 위 Space/Enter는 네이티브 클릭에 양보, `e.repeat`·조합키 무시, code가 빈 자동화 입력은 key로 보정) + 마우스 환경에서만 안내 문구 ⑤ 스와이프(Pointer Events, 12px에서 가로/세로 판정·90px 확정, `touch-action: pan-y`, setPointerCapture, 밀었다 놓은 뒤 따라오는 click 삼킴, 방향 태그 opacity). 코드는 Claude 작성, 사용자가 로컬 로그인·확인. **검증(로컬 bootRun + 브라우저 자동화, 470px 뷰포트)**: ① opacity 1·별 rgb(138,139,156) ② 읽기 19.74px / 단어 42.3 / 뜻 24 ③ 뒤집기에 speak("やくそく") 1회, 끄면 0회, 스위치↔localStorage↔설정 체크박스 동기 ④ D→알아요 전진·A→몰라요·←→ 이동·S→별표됨·Space→뒤집기(합성 이벤트로 확인 — 도구의 키 주입은 code가 비어 실제 키보드는 사용자 PC 확인) ⑤ 180px 우→알아요 5/6, 좌→몰라요 6/6, 30px는 무반응, 탭은 뒤집기. 부수: `.claude/launch.json` backend-dev가 JSON `\g` 이스케이프와 `NoDefaultCurrentDirectoryInExePath=1` 때문에 안 돌던 것 수리(`.\gradlew.bat` 절대경로 cd). **배포 검증(9/17 12:3x)**: PR #9 squash `283dd81` → run 35176421795 build·deploy success(앱 변경 감지·Docker build·전송·재기동·헬스·HTTP 스모크 전부 통과) → 컨테이너 `Up 2 minutes (healthy)` → 서빙 JS 해시 `index-sVamzaqn.js` = 로컬 빌드(배포 전 `index-BcUxcQes.js`). CI tests=197 failures=0. 실기기 손가락 스와이프·TTS 소리·실제 키보드는 사용자 확인(결과는 메모리 기록).
- **9/17 오후 추가 2건 (PR #10, 사용자 폰 확인 뒤 제안)**: 폰 실기기에서 스와이프·소리 정상("Quizlet 거의 따라옴"). ① 스와이프 확정 시 카드가 밀던 방향으로 화면 밖까지 날아감 — `.flying`(0.22s) + `flyOut()`이 220ms 뒤 `pick`, 다음 카드는 `key={cardId}`로 새 요소라 미끄러져 들어오는 전환이 없음. 첫 구현(한 프레임 플래그 + requestAnimationFrame)은 안 보이는 탭에서 rAF가 멈춰 `dragging`이 남는 걸 실측하고 교체. ② 한 화면 맞추기("스크롤이 거슬린다"): 학습은 몰입 모드(헤더·탭바 없음)라 `.study-shell{min-height:100dvh; flex column}`, 카드 `flex:1 1 auto; min-height:200px`(320 고정이 스크롤 원인), 아래 여백 100/96→40/24. 여백은 `.shell.study-shell`로 특이도를 올려야 했다 — 같은 특이도면 파일 뒤쪽 모바일 `.shell{padding … 96px}`가 이겨 96px 그대로였음(실측 후 수정). `max-height:720`에서 카드 패딩 축소·맨 아래 안내문 숨김. **검증(로컬 새 빌드, 브라우저 자동화)**: 넘침 0 — 375×812(카드 376, 전엔 304에 2px 넘침), 375×667(312, 안내문 숨김), 412×740(304), 1280×800(333, 가운데 정렬 유지). 305×510 극소 창만 120 넘침(카드 최소 200, 허용). 날아가기: 드래그 직후 `flying`+translateX(480px)·rotate, 1초 뒤 클래스 깨끗·다음 카드 포커스, 좌 스와이프 동일. **배포 검증(9/17 오후)**: PR #10 squash `75cd3c0` → run 35178789915 build·deploy success(전 단계 통과) → 컨테이너 healthy → 서빙 JS 해시 `index-CdiObcSS.js` = 로컬 빌드. CI tests=197 failures=0. 폰 실기기 한 화면·날아가기는 사용자 확인(메모리 기록).
- **9/17 오후 추가 3번째 (PR #11)**: 폰은 한 화면·날아가기 둘 다 정상(사용자 확인). PC는 여전히 스크롤 — 스크린샷 실측 카드 폭 981px = CSS 720px → 배율 1.36(윈도우 배율·브라우저 확대), CSS 기준 세로 ≈ 617px라 720 규칙만으론 부족. `max-height: 660px` 단계 추가: 셸 상하 여백 16, 카드 min 160·패딩 16·단어 40px, 액션·답 버튼 패딩 축소, `.study-tip` 안내문 숨김(클래스 신설), 여백 전반 축소. **검증(로컬 새 빌드)**: 넘침 0 — 1400×617(카드 346, 안내문 숨김, 단어 40px), 1400×580(309), 1400×700(296, 안내문 표시·52px), 1280×800(333), 375×812(376, 변화 없음). **배포 검증(9/17 오후)**: PR #11 squash `ad7eb20` → run 35179725025 build·deploy success → 컨테이너 healthy → 서빙 해시 `index-DWCWQT6f.js` = 로컬 빌드. CI tests=197. **9/17 총 PR 3개(#9·#10·#11) 배포, 9월 코드 종료.** 사용자 PC 확대 상태 확인은 메모리 기록.
- **9/17 저녁 점검 → PR #12 (사용자 요청 "문제 생긴 건 없는지 꼼꼼히")**: 운영 HTTP 200/401 정상, 서빙 JS·CSS에 오늘 기능 전부 포함, ERROR 0, Redis 연결 예외 1건(12:59, 재시작 2분 뒤 300ms 타임아웃 1회, fail-open, 이후 없음 — 프런트만 바꾼 오늘과 무관). **발견 1건**: `.study-shell`은 퀴즈·타이핑도 쓰는 공용 클래스라 PR #10·#11의 세로 채우기·여백·660 단계가 두 화면에도 적용됐음(깨짐은 없었으나 의도 밖) → 복습 셸에 `study-fit` 클래스 신설, 오늘 규칙 21개를 전부 `.study-fit`으로 이동, `.study-shell`은 원래의 max-width만 남김. 덤: 660 단계 단어 크기 `min(40px, 9vw)`(320px 폰에서 모바일 clamp보다 커지는 역전 방지). 검증: 퀴즈·타이핑 셸 `display:block`·아래 여백 100/96·`.study-fit` 없음(원상복구), 복습 1400×617·375×812 넘침 0. 측정 주의: Claude 창이 최소화되면 `.shell`의 fadeUp 애니메이션이 첫 프레임(translateY 6px)에 멈춰 넘침 6으로 보인다 — `getAnimations().finish()` 후 재면 0. **배포 검증(9/17 저녁)**: PR #12 squash `87bc95c` → run 35180868130 build·deploy success → 컨테이너 healthy → 서빙 해시 `index-DVCWBRDp.js` = 로컬 빌드. CI tests=197. **9/17 총 PR 4개(#9~#12). 9월 코드 종료, 다음 코드 11월.**
- **9/17 밤 PR #13 — 덱 목록 N+1 제거 (사용자 신고: "내 덱 새로고침하면 10초")**: 원인 = `DeckService.findAll`이 덱마다 `countByDeckId`(덱 169개 → 170쿼리) × 프런트가 한 페이지에서 `/decks`를 2번 호출(목록 + 사이드바 폴더 개수) = 페이지당 약 340쿼리. 실측: 운영 DB 카드 18,812장·`fk_cards_deck` 인덱스 있음·count 1회 0.45ms·200회 18ms·DB 호스트 유휴, 앱↔DB TCP 연결 5~8ms → DB가 느린 게 아니라 **왕복 횟수**가 원인(배포 직후 JVM 콜드면 더 김, 오늘 4번 배포). 8/14 백로그 ⓪ "cardCount N+1 잔존"이 이 항목. 수리 = 이미 있던 `CardRepository.countByDeckForUser`(통계 화면용 GROUP BY) 재사용 + `getOrDefault(…, 0L)` — 카드 0장 덱은 GROUP BY 결과에 줄이 없어 `get`이면 null→NPE. **사용자 타이핑**: "0장 덱이면 문제" 자력 답, `put`·`getOrDefault` 줄은 풀어 쓴 동치코드로 설명, 오타 2개(`d,getId()`·체인 중간 세미콜론 → 에러는 다음 줄을 가리킴). 회귀 테스트(Claude) `findAll_cardCountsWithoutNPlusOne`: Hibernate Statistics로 **쿼리 수를 직접 셈**(덱 4개에 2번 이하) + 0장 덱 0 검사. 테스트가 한 트랜잭션이라 `em.flush()` 후 통계를 비워야 INSERT가 안 섞임. **증명**: 수리만 stash → 그 테스트 1건만 실패("실제 5번"), 복구 후 10/10 (JDK 17). 남김: 프런트의 `/decks` 이중 호출은 그대로 — 호출당 2쿼리가 돼 급하지 않음(11월). **배포 검증(9/17 14:5x)**: PR #13 squash `7d6eb28` → run 35186657816 build·deploy success(전 단계 통과) → 컨테이너 healthy, `/app/` 200·비로그인 `/decks` 401. CI tests=198(회귀 테스트 +1) failures=0. 체감 속도는 사용자 확인.
- **9/17 PR #14 — 스와이프 비행 중 경합 수리 (Codex 검토 재현, PR #10 회귀, Claude 작성 코드의 결함)**: 카드가 날아가는 0.22초 동안 키보드·카드 터치는 막았지만 **버튼(몰라요·알아요·이전·다음)을 안 막았다**. 그 사이 버튼이 카드를 넘기면, 뒤늦게 도착한 `flyOut`의 예약 `pick`이 **옛 렌더의 클로저**(`idx` 옛값 + `setIdx(i=>i+1)`)로 한 번 더 넘김. **재현(수리 전, 운영과 같은 번들 `index-DVCWBRDp.js`)**: ① 1번 카드 우 스와이프 직후 `몰라요` → 1→3 두 칸 이동, 1번 카드 표시가 "알아요"로 덮어써짐 ② 5/6에서 스와이프 직후 `다음` → idx 6 → 카드도 패널도 없는 **빈 화면**. 수리: `pick`·`goNext`·`goPrev`에 `flying.current` 가드(즉시 적용되는 실제 잠금) + 버튼 `disabled={flyingCls}`(한 프레임 뒤 붙는 표시) + 늦은 채점은 `currentCardId` ref가 날아간 카드와 같을 때만 적용(이중 안전) + 언마운트 시 타이머 정리. **검증(수리 후 `index-6MSSahd_.js`, 같은 스크립트)**: ① 비행 중 `몰라요` 클릭·A키·→키 셋 다 무시, 1→2 한 칸 ② 5/6→6/6 카드 정상 ③ 마지막 카드 좌 스와이프 → "제출할까요?" ④ 60ms 시점 카드 `flying`·버튼 3개 disabled, 종료 후 해제. 재현 도구: 합성 PointerEvent(포인터 캡처 3함수를 no-op으로 패치) + 같은 태스크 안에서 버튼 click → 220ms 창 안에 확정적으로 들어감. Codex 2번 지적("덱 목록 최적화 미연결")은 사용자가 Map만 치고 `.map` 줄을 바꾸기 전 중간 상태를 본 것 — 그 뒤 완성·커밋(`f1682c4`)·PR #13로 배포됨. 교훈: 시간차가 있는 예약 작업(setTimeout)은 ⓐ 그동안 **모든** 입력 경로를 잠갔는지 경로별로 세고 ⓑ 도착 시점에 대상이 아직 같은지 확인한다. **배포 검증(9/17 15:1x)**: PR #14 squash `fff008a` → run 35187298120 build·deploy success → 컨테이너 healthy → 서빙 해시 `index-6MSSahd_.js` = 로컬 빌드. CI tests=198. **9/17 총 PR 6개(#9~#14): 기능 2·화면 맞춤 1·누수 격리 1·성능 1·경합 수리 1.**
- **9/25 A1 수리 → PR #15 (사용자 "A1 해", 보안 범주 = 9/1 규칙상 동결 예외)**: 9/5 결정대로 재현부터. 방법 = 워크트리 코드를 로컬 bootRun(격리 DB `vocamaster_a1`·격리 Redis 6380·포트 8081, 개발 DB 무접촉) + Edge + 지연 프록시(8082 — 서버 처리 순서는 그대로 두고 브라우저에 오가는 시점만 붙잡음) + 같은 계정의 두 번째 기기 B. **수정 전 실측**: ① 서버가 갱신을 먼저 처리하고 그 응답이 로그아웃 응답보다 늦게 도착 → 로그아웃은 옛 쿠키 A0을 들고 나가 무효과(`revokeIfActive` 0행), 늦은 응답이 새 쿠키 A1(14일)·access 토큰을 되살림, DB A1 살아 있음, `/app/` 재진입 시 로그인 상태 = **로그아웃 취소** ② 서버가 로그아웃을 먼저 처리 → 갱신이 폐기된 A0으로 가 재사용 판정 → `revokeAllByUserId` → **B까지 로그아웃**(B 갱신 401). 9/5 분석의 ⓑ와 반대 순서를 둘 다 실증. **수리(프런트 3파일, +34/-8)**: `client.ts` 공용 `logout()` — 진행 중인 갱신을 기다린 뒤(새 쿠키가 심긴 뒤) 로그아웃 / 로그아웃 중 새 갱신 금지(`loggingOut`) / 로그아웃 전에 출발한 갱신 응답은 토큰 저장 안 함(`authEpoch`) / 로그아웃 중 401이면 `api()`가 `location.href` 이동을 안 함(이동하면 나가는 중인 로그아웃 요청이 끊긴다 — 설계 중 발견) / TopNav·Settings 로그아웃 두 벌 통합. **수정 후**: ①② 모두 로그아웃이 A1을 들고 나가 폐기, 토큰·쿠키 없음, B 유지(갱신 200). 평소 흐름 회귀 5/5(평범한 로그아웃 60ms·평범한 갱신·갱신 실패→로그인 화면·설정 화면 로그아웃·로그아웃 뒤 같은 탭 재로그인 후 갱신), 페이지 오류 0, oxlint 0, build 통과. 측정 주의: 링크 클릭 같은 SPA 전환 뒤의 `networkidle` 대기는 즉시 통과한다 — 갱신이 끝나기 전에 결과를 읽어 R5가 거짓 실패했고, 브라우저 요청을 직접 관찰한 대조 실험으로 스크립트 문제를 확정한 뒤 갱신 응답 대기로 고쳤다. 스크립트는 `.codex-resume-audit/a1-*`(gitignored, 로컬 재실행용 — 프런트 자동 테스트 도구가 없어 CI 밖). 범위 밖: 비번 변경·탈퇴는 서버가 그 계정의 refresh 토큰을 전부 폐기해 14일 부활 없음(늦은 갱신이 access ≤1h를 되살릴 여지만), 서버 쪽 방어(로그아웃이 회전된 토큰의 후속까지 폐기)는 토큰 계보 저장이 필요해 미적용. **배포 검증(9/25 15:44)**: PR #15 squash `59433b7` → run 36103273649 build·deploy success(앱 변경 감지·Docker build·전송·재기동·헬스·HTTP 스모크 전부 통과) → 컨테이너 `vocamaster-app-app-1 Up 2 minutes (healthy)` → 서빙 JS 해시 `index-BNbEOE2r.js` = 로컬 빌드(배포 전 `index-6MSSahd_.js`). CI tests=198 failures=0. 운영 `/app/login` 200·비로그인 `/decks` 401.
- **9/25 사용자 요청 마감 — 알아요/몰라요 표시**: 카드 안의 상태 배지를 없애고 카드 묶음 바깥에 주황색 몰라요/녹색 알아요 제목과 개수를 표시. 기존 연속 정답이 3회 이상이면 알아요, 나머지는 몰라요(미학습 포함), 오답이면 즉시 초기화. 카드 API에 본인 진도를 페이지별 일괄 조회하고 복습·퀴즈·타이핑 결과를 기존 공통 진도에 연결. DB 구조·복습 간격 변경 없음. 듣기 저장·복구 확장은 제외하여 기존 기록 없는 연습 모드 유지, 과거 퀴즈·타이핑 이력 소급 합산 없음. 검증: 로컬 전체 테스트 220/220·bootJar·frontend lint 통과, 모의 API Chrome 화면 검사(PC/390px·빈 그룹/덱·조회 재시도·104장) 통과. 이 범위를 마감한 뒤 면접·이력서 준비로 전환하며 새 기능은 11월에 재평가.
...
- [x] **[SHOULD]** 비밀번호 변경 시 모든 refresh token 폐기 (비번 변경 API 추가 시 처리) ✅ (9/25 코드 확인 — `UserService.changePassword` 끝의 `revokeAllByUserId`. 다른 기기의 access는 만료(1h)까지 유효)
...
- [ ] **React 후속 백로그 (2026-08-14, Codex 재검산 — 8/19 ②③④ 완료·likedByMe/mine/공개 카드 API/상세 화면/UNLISTED 비로그인 열람/무효토큰 401)**: ① ~~Explore 로딩 상태·페이지네이션(현재 30개 상한)~~ **완료** (9/25 확인: '불러오는 중' 표시 + 페이지 교체) ⓪ ~~목록의 cardCount는 여전히 덱마다 count 1번(N+1 잔존 — likedByMe만 IN으로 해결됨, 다음 정리 후보)~~ **완료 9/17 PR #13** (170→2쿼리) ⑤ ~~Gradle-frontend 연결~~ 완료 8/23 (frontendBuild → processResources, bat errorlevel 중단) ⑥ ~~통계 화면용 API~~ 완료 8/23 ⑦ ~~OSIV 명시 off + 스프링 기본 계정 소음 정리~~ **완료 8/25** (open-in-view false·기본 계정 자동구성 제외) ⑧ bat이 서버 준비 후 브라우저 열기 ⑨ ~~Card에 읽기(reading) 필드~~ **완료 8/23 (V14)** — 생성·수정·복사·3칸 가져오기(단어|읽기|뜻)·학습·퀴즈·타이핑·목록 표시, 🔊는 읽기 우선. 채점엔 미사용(옵션 후보) ⑩ ~~목업 옵션들~~ 설정 화면으로 완료 8/23 ⑫ ~~퀴즈 선택지 수 옵션~~ 완료 8/25 ⑬ **연속 재생 플레이어**(덱 단어를 N회·M초 간격·속도 조절로 쭉 읽어주기 — 순수 프론트 반나절, TTS 재활용, 오디오 파일 불필요) ⑭ ~~듣기 받아쓰기~~ **구현 완료 8/28** (/listening/:deckId — TTS 3회 onend 체인(speakTimes), 스펠링=단어·읽기 양쪽 정답, 뜻=쉼표 후보, v1 연습 모드. 후속: 세션 기록 저장) ⑮ **독해 도메인 — 11월 이후 ㉓(AI 생성)와 결합 확정** (AI가 덱 수준 맞춤 지문+4지선다 생성 → 콘텐츠 병목 해소 + LLM 통합 이력서 줄. 단독 구현은 빈 기능 위험이라 기각, 8/28) ⑪ ~~구형 Mustache 전체~~ **청소 완료 8/28** — 템플릿 4장·PageController·QuizController(구형 4 API)·WrongNote 패키지·구형 dto 5종·mustache 의존성 제거. **부수 수리: 덱 통계 퀴즈 항을 attempts→세션(quiz_questions) 기반 전환** (React 퀴즈가 통계에 안 잡히던 잠복 버그). 테스트 175→166(구형 9개 순장) ⑯ DeckDetail 별표 연타 가드(카드 id별 pending — 더블클릭 시 toggle 2번 경합, Codex 8/26) ⑰ Quiz 자동 넘김 타이머 unmount cleanup(useEffect 정리 함수 — 1초 내 화면 이탈 시 늦은 콜백) ⑱ TTS 버튼 focus-visible 링(outline:none 제거 말고 별도 링 — 키보드 접근성) — **9/25 확인: 미해결** (일반 `.speak-btn`은 outline:none + 확대만, 학습 화면 버튼만 색 테두리) ⑲ StudyService.getDeckStats 세션별 records N+1 → count/sum 집계 쿼리 전환(8/26 @Transactional 수리 때 성능은 백로그로 분리) ⑳ CI에 docker build 검증 추가 여부(현재 lint+compose config만 — 빌드 5분+ 비용 대비 검토, CD 구축 때 재결정) ㉑ 프론트 자동 테스트(숫자키·자동넘김·설정 localStorage — 백엔드 172개가 보증 못 하는 상태 흐름) ㉒ ~~다중 파일 일괄 가져오기~~ **완료 8/27** (/import-files — 파일명=덱명, BOM·전각괄호 전처리 실측, 순차 등록+결과표, 백엔드 무변경) ㉓ **AI 카드 자동 생성**(Gemini 무료 API — 단어만 치면 읽기·뜻·예문, NewsPick 요약과 같은 HTTP+프롬프트 패턴, 1~2세션, 이력서 'LLM 통합')
- [x] 익명 요청 거부를 401로 통일 ✅ (9/25 확인: 이후 수리되어 SecurityConfig `authenticationEntryPoint`가 401 — 아래는 당시 기록) — SecurityConfig에 authenticationEntryPoint 미설정이라 현재 실측 403 (PublicDeckHttpTest가 박제). 프론트가 "로그인 필요"와 "권한 없음"을 구별하려면 401 + ErrorResponse JSON이 맞음
...
- [x] **[MUST]** React + TypeScript + Vite 프로젝트 셋업 (NewsPick 스택과 동일) ✅ (8월 완료, 9/25 체크 정리)
- [x] **[MUST]** Spring 빌드에 React 결과물 번들 (`src/main/resources/static/react/`) ✅ (실제 경로 `static/app/`, Gradle `frontendBuild` → processResources 8/23)
...
- [x] **[MUST]** httpOnly Cookie + access token 자동 갱신 인터셉터 ✅ (`client.ts` single-flight 갱신, 9/25 A1로 로그아웃 경합까지 수리)
- [x] **[MUST]** TTS 버튼 통합 (위 항목과 결합) ✅ (8월 완료, 9/25 체크 정리)
- [x] ~~**[SHOULD]** README에 "Mustache + React 공존 이유" 명시 (ADR-016 참조)~~ — 해당 없음 (8/28 Mustache 전체 청소)
