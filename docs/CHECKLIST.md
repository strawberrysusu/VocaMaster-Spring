...
| 항목 | 값 |
|---|---|
| **진행 중인 Phase** | **🧊 기능 동결 (8/28) — 4 학습 모드(플래시카드·퀴즈·타이핑·듣기)·공개 운영·CD·k6 실측 완비.** 이후: 유지보수 + 마감재(week note·Mustache 청소)만. 해동 조건: 11월 산업기사 후 (독해+AI 콤보 등) |
...
- 새 기능 떠오르면 해당 Phase 하단 `### 🧊 기능 동결 선언 (2026-08-29)

**동결 조건 전부 충족 후 확정** — Codex 3차 검산 통과 기준:
- 보안: 구글 선점(pre-hijacking) 차단 · 탈퇴 정합(JWT 즉시 차단·공개 콘텐츠 숨김·복사/좋아요 우회 404·캐시 count/rebuild 제외)
- 데이터: V19 임포트 오염 8,124장 수리 (prod 실측 잔존 0) + 전처리 3형식(CRLF·n1형 포함)
- 안정화: import 원자성(단일 트랜잭션 API) · update @Transactional · PATCH @Valid · CI push 전체 diff
- 마지막 기능: 📁 폴더(V20, SET NULL) + 덱 일괄 삭제 — 테스트 **177** 전부 그린
...
### 🆕 추가 아이디어`에 적기. 본 리스트에 즉시 끼워넣지 말 것
