...
- ~~**다음 주 묶음 A**~~ → 9/3 완료. **다음 주 = B**: Claude Design 모바일 프로토타입(9/3 `VocaMaster 리디자인.html`, 375 토글) 중 프론트만으로 되는 3개 — 카드 상자화·하단 고정 학습 버튼·모드 세그먼트. 프로토타입의 카드별 「박스 N·다음 복습」 칩과 덱 진행바는 **백엔드 필요 → 11월**.
- **작업 중 발견, 구현 안 함 (백로그)**: ⓐ 덱 상세 **카드 수정 행**(`.row-edit`)은 375px에서 입력칸 3개가 40px 폭으로 짜부돼 사실상 입력 불가 — 이번 범위(일반 행) 밖이라 미수정. ⓑ 덱 상세 **카드 추가 폼**(`.create-row`)이 모바일에서 세로 397px로 늘어져 목록이 한 화면 아래로 밀림. 둘 다 모바일 전용 CSS 결함, 다음 묶음 후보. ⓒ 프리뷰 에뮬레이터에서 폭 768.57px(소수)일 때 `max-width:768`·`min-width:769` 둘 다 불일치 — 실기기 정수 폭에선 해당 없음(환경 특성, 기록만).
