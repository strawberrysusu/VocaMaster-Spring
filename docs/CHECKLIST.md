...
- **그때까지 회피**: 보내는 중엔 버리기 X / 텍스트 가져오기는 미리보기 **행**을 확인(숫자만 X) / 빈 칸 섞인 시트 데이터는 파일 가져오기 X.
