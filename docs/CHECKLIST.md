...
- **동결 판정 (Codex, Claude 동의)**: 계획은 PR #5 하나였는데 같은 날 #6·#7까지 3개 배포. 수정 자체는 정상이나 **동결 장치가 또 풀림** — 이번엔 "결함"이라는 사유로. 규칙 보강 제안: **상한 밖 '장애'는 모든 기기에서 기능이 안 되는 것만**, 특정 화면 폭의 결함은 주간 묶음에 포함. **다음 주 코드 변경 0(이번 주 초과분 상쇄), B(카드 상자화·하단 고정 버튼)는 11월 재평가로.** ← 사용자 결정 대기.
