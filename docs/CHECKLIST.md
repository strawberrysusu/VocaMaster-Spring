...
- **9/17 저녁 점검 → PR #12 (사용자 요청 "문제 생긴 건 없는지 꼼꼼히")**: 운영 HTTP 200/401 정상, 서빙 JS·CSS에 오늘 기능 전부 포함, ERROR 0, Redis 연결 예외 1건(12:59, 재시작 2분 뒤 300ms 타임아웃 1회, fail-open, 이후 없음 — 프런트만 바꾼 오늘과 무관). **발견 1건**: `.study-shell`은 퀴즈·타이핑도 쓰는 공용 클래스라 PR #10·#11의 세로 채우기·여백·660 단계가 두 화면에도 적용됐음(깨짐은 없었으나 의도 밖) → 복습 셸에 `study-fit` 클래스 신설, 오늘 규칙 21개를 전부 `.study-fit`으로 이동, `.study-shell`은 원래의 max-width만 남김. 덤: 660 단계 단어 크기 `min(40px, 9vw)`(320px 폰에서 모바일 clamp보다 커지는 역전 방지). 검증: 퀴즈·타이핑 셸 `display:block`·아래 여백 100/96·`.study-fit` 없음(원상복구), 복습 1400×617·375×812 넘침 0. 측정 주의: Claude 창이 최소화되면 `.shell`의 fadeUp 애니메이션이 첫 프레임(translateY 6px)에 멈춰 넘침 6으로 보인다 — `getAnimations().finish()` 후 재면 0. 배포 검증은 아래 추가.
