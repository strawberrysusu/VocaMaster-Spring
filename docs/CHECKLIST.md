...
| ② `detectSeparator` 오분할이 실패로 안 잡힘 | 2~4h | ✗ (버그) | 미룸 |
| ③ 파일 가져오기 미리보기·수동 구분자 | 반나절~하루 | ✓ | 미룸 |
| ④ `lastReviewedAt` DTO 노출 | small~medium | ✓ | **하지 않음** |
| ⑤ 기기·세션 관리 | **2~4일** | ✓ | 미룸 (9월 착수 금지) |

- **② detectSeparator** — `ImportService.java:158-177`. 후보 고정 순서(탭→|→:→,→-), 표본은 **비어있지 않은 앞 5줄뿐**, 과반 미달 시 **하이픈으로 강행**(:35, :176).
  실측 재현(감지·파싱 로직 복제 실행): `"mother-in-law 시어머니"` → front=`mother` / reading=`in` / back=`law 시어머니` 로 **SAVED**.
  `shapeOk`(:141)가 조각 수 2·3이면 통과시켜서 **오분할이 failed로 보고되지 않는다**. 4조각 이상일 때만 실패.
  → 최소 수리는 "과반 미달 시 하이픈 폴백 대신 400" 한 줄. 전면 개선은 회귀 테스트 설계가 비용의 대부분
  (현 `ImportServiceTest.java:95-107`은 정상 케이스만 검증).
  **그 전까지 낯선 CSV는 미리보기가 있는 '일괄 붙여넣기'(ImportCards)로 넣는다.**
...
> 브레이크는 문구가 아니라 숫자로 둔다 — **주당 VocaMaster PR 상한 1개, 항목당 타임박스 1시간.**
