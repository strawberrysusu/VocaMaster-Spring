...
| 항목 | 값 |
|---|---|
| **진행 중인 Phase** | **Phase 3 완전 졸업 🎓 (2026-08-02 — 완료 기준 5/5)** → Phase 4 진입 대기 |
...
| **▶ 다음 액션 (resume)** | **🏁 Phase 3 졸업 시험 1교시 끝 (2026-07-31 — 데모·동시성·MUST 전부 ✅)** — 2교시 남음: ① due 복합 인덱스 자기 말 재설명 (오늘 첫 수업 들음) ② 면접 질문 5 리콜 (`review-algorithm.md` 읽고 → 안 보고). 통과 시 **Phase 4 (공개 단어장) 진입**. **미니 정비 3건 완료 (8/2 — P1-1 인증 롤백[감지/제재 시간 분리] ✅, Import 트랜잭션 ✅, 화면 로그아웃 ✅)**. 다음: 졸업 2교시 → **README 정비 30분**(겨울 지원 시즌 첫 화면). 개강 전 목표: **Phase 4 MUST 절반** (주3 8/17~은 국비 행정으로 물량 축소). 밀린 것: week note(최고가) + 프로그래머스 + 폐쇄훈련 #3 3회차(빈칸 채우기, 주말). ⚠️ 터미널 gradlew 죽으면 JDK 25 Lombok 함정 참조 |
