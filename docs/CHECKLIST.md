...
- **다음 주 묶음 A (9/8~, 사용자 확정, 합계 1시간)**: ① 폰 카드 수정 행 입력칸 세로 배치 ② 폰 카드 추가 폼 늘어짐 수리 ③ 가져오기 아래쪽 등록 버튼 제거. **그다음 주 B**: Claude Design 모바일 프로토타입(9/3 `VocaMaster 리디자인.html`, 375 토글) 중 프론트만으로 되는 3개 — 카드 상자화·하단 고정 학습 버튼·모드 세그먼트. 프로토타입의 카드별 「박스 N·다음 복습」 칩과 덱 진행바는 **백엔드 필요 → 11월**.
