...
**알려진 한계 — 새 제출은 이미 반영된 카드를 한 번 더 센다.** 클라이언트는 앞선 제출에 어떤 카드가 들어 있었는지 알 방법이 없어 전부 다시 보낸다. 스모크에서 실측: 3장 제출(응답 유실) → 4번째 답하고 새 제출 → 그 3장이 box 2에서 **box 3**으로, `correct_streak` 2. 영구 데드락보다는 낫다고 판단해 감수하되, **화면에 그 사실을 적고 '버리기'라는 반대 선택지를 함께 둔다.** 자기평가형 SRS라 한 칸 과승급은 다음 회차에 스스로 교정된다.
...
- 막다른 골목 두 시나리오 재현 후 탈출 확인 — due 모드(고아 감지), 덱 모드(409 신호)
